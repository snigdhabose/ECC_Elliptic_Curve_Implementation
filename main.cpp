...
int main(int argc, char *argv[])
{
    ...
    cout << "Elliptic Curve Implementation - cryptography\n";
    ...
    cout << "A's public key Pa = " << a << "*" << G << " = " << Pa << endl;

    // Bob
    int b = irand(1,myEllipticCurve.Degree()-1);;
    ec_t::Point Pb = b*G;  // public key
    cout << "B's public key Pb = " << b << "*" << G << " = " << Pb << endl;

    // Jane, the eavesdropper
    int j = irand(1,myEllipticCurve.Degree()-1);;
    ec_t::Point Pj = j*G;
    cout << "J's public key Pj = " << j << "*" << G << " = " << Pj << endl;

    cout << "\n\n";

    // A encrypts her message to send to B
    ...
    // encrypt using B`s key
    ec_t::Point Pk = a*Pb;
    ec_t::ffe_t c1( m1*Pk.x() );
    ec_t::ffe_t c2( m2*Pk.y() );

    // encrypted message is: Pa,c1,c2
    cout << "Encrypted message from A to B = {Pa,c1,c2} = {" << Pa << ", " << c1 << ", " << c2 << "}\n\n";

    // B now decrypts A`s message, using her public key and his session integer "b" which was also used to generate his public key
    Pk = b*Pa;
    ec_t::ffe_t m1d = c1/Pk.x();
    ec_t::ffe_t m2d = c2/Pk.y();

    cout << "\tB's decrypted message from A = (" << m1d << ", " << m2d << ")" << endl;
    ...
    cout << "\nJ's decrypted message from A = (" << m1d << ", " << m2d << ")" << endl;
